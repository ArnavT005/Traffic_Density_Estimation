...
int runTime_avgOver = 3, sleep_time = 10;
...
int main(int argc, char** argv) {
    ...
    if(method_number==3){
        if(!isInteger(argc!=5 || !isInteger(argv[3]) || stoi(argv[3])<=0 || !isInteger(argv[4]) || stoi(argv[4])<=0) { invalidargs(); return 0;}
        vector<int> temp = {stoi(argv[3]), stoi(argv[4])}
        p3p4.push_back(temp);
    }
    if(method_number==4){
        if(!isInteger(argc!=4 || !isInteger(argv[3]) || stoi(argv[3])<=0) { invalidargs(); return 0;}
