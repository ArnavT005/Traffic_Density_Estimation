...
void M4_spatialSplit(cv::VideoCapture video, cv::Mat background, cv::Mat matrix, int x, ofstream &file) {
    ...
    //divide background
    int start = 0, end = background.rows/x;
    for ( int i = 0; i < x; i++ ){
        arg[i].background = background(Range(start, end), Range(0, background.cols)).clone();
        start = end + 1;
        end = (i+2)*background.rows/x;
    }
    ...
    while (true) {
        ...
        start = 0; end = frame1.rows/x;
        //Queue Density - subtract background
        for ( int i = 0; i < x; i++ ){
            arg[i].frame1 = frame1(Range(start, end), Range(0, frame1.cols));
            arg[i].frame2 = frame2(Range(start, end), Range(0, frame2.cols));
            start = end + 1;
            end = (i+2)*frame1.rows/x;
        }
