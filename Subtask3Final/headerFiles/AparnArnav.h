...
void* M5(void* arg) {
    ...
    int end = args -> end;
    cv::resize(background, background, Size(280, 700));
    ...
    int see_every_n_frame = 5, frame = start + 1;

    //read Frame 1
    video.read(frame1);
    frame1 = warpAndCrop(frame1, matrix);
    cv::resize(frame1, frame1, Size(280, 700));
    ...
    while (frame <= end) {
        ...
        cv::resize(frame2, frame2, Size(280, 700));
